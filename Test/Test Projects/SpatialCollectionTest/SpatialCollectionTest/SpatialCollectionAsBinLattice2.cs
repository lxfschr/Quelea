...
namespace Agent
{
  ...
  public class SpatialCollectionAsBinLattice2<T> : ISpatialCollection<T> where T : class
  {
    ...
    public ISpatialCollection<T> getNeighborsInSphere(T item, double r)
    {
      ...
      List<LinkedList<T>> possibleNeighbors = getBins(item, r);
      ISpatialCollection<T> neighbors = new SpatialCollectionAsList<T>();
      foreach (LinkedList<T> bin in possibleNeighbors)
      {
        foreach (T other in bin)
        {
    ...
    private List<LinkedList<T>> getBins(T item, double radius)
    {
      List<LinkedList<T>> possibleNeighbors = new List<LinkedList<T>>();
      Point3d p = ((IPosition)item).getPoint3d();
      int col = (int)(p.X - min.X) / this.binSize;
      int row = (int)(p.Y - min.Y) / this.binSize;
      int layer = (int)(p.Z - min.Z) / this.binSize;
      ...
      for (int c = firstCol; c < lastCol; c++)
      {
        for (int r = firstRow; r < lastRow; r++)
        {
          ...
          {
            possibleNeighbors.Add(this.lattice[c][r][l]);
          }
        }
      }
